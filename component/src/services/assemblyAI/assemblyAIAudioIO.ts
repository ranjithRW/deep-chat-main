...
export class AssemblyAIAudioIO extends DirectServiceIO {
  ...
  override async extractResultData(result: AssemblyAIResult): Promise<Response> {
    ...
    if (!result.upload_url) throw new Error('AssemblyAI did not return an upload URL');
    const key = this.connectSettings?.headers?.[AUTHORIZATION_H];
    if (!key) throw new Error('AssemblyAI authorization key is missing');
    const pollingResult = await ASSEMBLY_AI_POLL(key as string, result.upload_url);
